...
;$(function(){
    ...
    $('.ajax-post').click(function(){
		...
		// form		为表单对象
		// target	为目标地址
		// query	为提交信息的序列化，?a=b&c=d
		
		// <form         class="form-horizontal">
		// <button target-form="form-horizontal">确 定</button>
		// 表单的 form 中的 class="" 和提交按钮的 target-form="" 引号中的内容要一样
        var target_form = $(this).attr('target-form');
        var that = this;
        var nead_confirm=false;
		
		// 事件优先级：1.表单提交   2.链接   3.DIV块
        if( ($(this).attr('type')=='submit') || (target = $(this).attr('href')) || (target = $(this).attr('url')) ){
            form = $('.'+target_form);

            if ($(this).attr('hide-data') === 'true'){//无数据时也可以使用的功能
            	form = $('.hide-data');
				// form.serialize()  序列化表单， 变为：FirstName=Bill&LastName=Gates
				// 如： $("div").text($("form").serialize());
